...
Thus we can focus on inhomogeneous intensity. A spatially varying intensity
can also be estimated non-parametrically by kernel estimation, which places kernels' on each datapoint (often bi-variate Gaussian) and optimizes the bandwidth.(Baddeley, A., Rubak, E. & Turner, R. (2015). Spatial point patterns: methodology and applications with R. CRC press) We plotted kernel estimated intensity for ppp object as well as the hot spot with the kernel estimate to identify areas of elevated intensity.
